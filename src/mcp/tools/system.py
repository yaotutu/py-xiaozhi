...
async def get_device_status(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        status = {
            "audio_speaker": {
                "volume": (getattr(app.display, 'current_volume', 50)
                           if app.display else 50),
                "muted": (getattr(app.display, 'is_muted', False)
                          if app.display else False)
            },
            "screen": {
            ...
            "device_state": (app.device_state.name
...
async def set_brightness(args: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if platform.system() != "Darwin":
            logger.warning(
                f"[MCP] 非Mac系统 ({platform.system()})，无法控制屏幕亮度")
            return False
...
async def get_brightness(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        return json.dumps({
            "success": False,
            "platform": (platform.system() if 'platform' in locals() 
                         else "Unknown"),
            "error": str(e),
            "fallback_data": {
                "brightness": 80,
...
async def get_battery_info(args: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.info(
            f"[MCP] 电池信息获取成功: {battery_info.get('percentage', 'N/A')}%")
