...
class AudioCodec:
    ...
    async def _create_resamplers(self):
        """
        创建输入重采样器，转换设备采样率到16kHz 输出固定24kHz，不需要重采样.
        """
    ...
    async def _create_streams(self):
        """
        创建音频输入输出流 输入流使用设备原生采样率，输出流固定24kHz.
        """
        try:
            # 录音流
    ...
    def _input_callback(self, indata, frames, time_info, status):
        """
        录音回调函数 处理录音数据，重采样到16kHz并进行Opus编码.
        """
    ...
    async def write_audio(self, opus_data: bytes):
        """
        解码Opus音频数据并放入播放队列 输出24kHz PCM数据，直接用于播放.
        """
        try:
            # Opus解码为24kHz PCM数据
