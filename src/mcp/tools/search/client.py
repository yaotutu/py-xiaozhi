...
class SearchClient:
    ...
    async def search_bing(self, query: SearchQuery) -> List[SearchResult]:
        ...
        try:
            if not self.session:
                raise RuntimeError("SearchClient not initialized. Use 'async with' statement.")
    ...
    async def _parse_search_results(self, html: str, query: SearchQuery) -> List[SearchResult]:
        """
        解析搜索结果页面.
    ...
    async def _extract_webpage_content(self, html: str, url: str, max_length: int) -> str:
        """
        从HTML中提取主要内容.
        ...
        # 移除不需要的元素
        for tag in soup(["script", "style", "iframe", "noscript", "nav", "header", "footer"]):
            tag.decompose()
        
        # 移除具有特定类名的元素
        for selector in [".ad", ".advertisement", ".sidebar", ".nav", ".header", ".footer"]:
