...
def list_running_applications(filter_name: str = "") -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            try:
                ...
                if apps:
                    logger.info(f"[WindowsKiller] PowerShell扫描成功，找到 {len(apps)} 个进程")
    ...
    if not apps:
        try:
            ...
            if apps:
                logger.info(f"[WindowsKiller] tasklist扫描成功，找到 {len(apps)} 个进程")
                return _deduplicate_and_sort_apps(apps)
...
def kill_application_group(apps: List[Dict[str, Any]], app_name: str, force: bool) -> bool:
    """
    按分组关闭Windows应用程序.
...
def kill_application(pid: int, force: bool) -> bool:
    ...
    try:
        ...
        if success:
            ...
            logger.warning(f"[WindowsKiller] 关闭应用程序失败，PID: {pid}, 错误信息: {result.stderr}")
