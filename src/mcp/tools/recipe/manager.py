...
from .models import Recipe, RecipeSession, PaginatedResult
...
class RecipeManager:
    ...
    async def get_all_recipes(self, page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        获取所有菜谱（分页）.
    ...
    async def get_recipes_by_category(self, category: str, page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        根据分类获取菜谱（分页）.
    ...
    async def search_recipes(self, query: str, page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        搜索菜谱（分页）.
    ...
    async def recommend_meals(self, people_count: int = 2, meal_type: str = "dinner", page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        推荐菜品（分页）.
        ...
        if meal_type == "breakfast":
            filtered_recipes = [r for r in all_recipes if "早餐" in r.category or "早餐" in r.name]
        elif meal_type == "lunch":
            filtered_recipes = [r for r in all_recipes if "午餐" in r.category or "主食" in r.category]
        elif meal_type == "dinner":
            filtered_recipes = [r for r in all_recipes if "晚餐" in r.category or "荤菜" in r.category or "素菜" in r.category]
        else:
    ...
    async def what_to_eat(self, meal_type: str = "any", page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        随机推荐菜品（分页）.
