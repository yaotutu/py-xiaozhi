...
def get_frame_duration() -> int:
    ...
    try:
        ...
        if not is_official_server(config.get_config("NETWORK.WEBSOCKET_URL")):
            return 60
        p = pyaudio.PyAudio()
        # 获取默认输入设备信息
...
class AudioConfig:
    ...
    OUTPUT_SAMPLE_RATE = 24000 if is_official_server(config.get_config("NETWORK.WEBSOCKET_URL")) else 16000  # 输出采样率
    CHANNELS = 1
