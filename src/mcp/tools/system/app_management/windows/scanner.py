...
def scan_installed_applications() -> List[Dict[str, str]]:
    ...
    try:
        ...
        logger.info(f"[WindowsScanner] 从开始菜单扫描到 {len(start_menu_apps)} 个主要应用")
    ...
    try:
        ...
        for app in registry_apps:
            if app['display_name'].lower() not in existing_names:
                apps.append(app)
        logger.info(f"[WindowsScanner] 从注册表扫描到 {len([a for a in registry_apps if a['display_name'].lower() not in existing_names])} 个新的主要应用")
    except Exception as e:
    ...
    logger.info(f"[WindowsScanner] Windows应用扫描完成，总共找到 {len(apps)} 个主要应用程序")
    return apps
...
def _should_include_process(image_name: str, window_title: str) -> bool:
    ...
    system_processes = {
        ...
        "services.exe", "svchost.exe", "explorer.exe", "taskhostw.exe",
        "conhost.exe", "dllhost.exe", "rundll32.exe", "msiexec.exe",
        "wininit.exe", "lsm.exe", "spoolsv.exe", "audiodg.exe"
    }
