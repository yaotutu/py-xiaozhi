...
class AmapManager:
    ...
    async def direction_transit(self, origin: str, destination: str, 
                               city: str, cityd: str) -> Dict[str, Any]:
        """公交路径规划"""
        client = await self.get_client()
        try:
                    ...
                    "transits": [
                        {
                            ...
                            "segments": [
                                {
                                    "walking": {
                                        "distance": segment.walking.distance if segment.walking else 0,
                                        "duration": segment.walking.duration if segment.walking else 0
                                    }
    ...
    async def distance(self, origins: str, destination: str, 
                      distance_type: str = "1") -> Dict[str, Any]:
        """距离测量"""
        client = await self.get_client()
        try:
