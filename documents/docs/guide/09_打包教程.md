...
# 执行打包命令，指定小智客户端目录和配置文件
python main.py 路径/到/py-xiaozhi --config 路径/到/py-xiaozhi/build.json
```
...
#### macOS平台
```bash
python3 main.py 路径/到/py-xiaozhi --config 路径/到/py-xiaozhi/build.json
```
...
使用UnifyPy，您可以一键为小智客户端生成跨平台的安装包和可执行文件。只需准备好依赖环境和配置文件，即可轻松实现项目的分发和部署。

对于高级用户，UnifyPy还提供了丰富的定制选项，可以根据需求调整打包参数和生成格式。通过本教程中提供的配置说明和最佳实践，您应当能够顺利完成小智客户端的打包工作。 