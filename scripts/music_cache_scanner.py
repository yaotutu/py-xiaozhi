...
"""
音乐缓存扫描器
扫描cache/music目录中的音乐文件，提取元数据，生成本地歌单
...
class MusicCacheScanner:
        ...
        for i, file_path in enumerate(music_files, 1):
            ...
            try:
                ...
                if metadata.extract_metadata():
                    ...
                    print(f"   ✅ {display_title} - {display_artist} ({metadata.format_duration()})")
                else:
    ...
    def sort_playlist(self, sort_by: str = "artist"):
        """排序歌单"""
        sort_functions = {
            "artist": lambda x: (x.artist or "Unknown", x.album or "Unknown", x.title or "Unknown"),
    ...
    def print_playlist(self, limit: int = None):
        ...
        for i, metadata in enumerate(self.playlist[:limit] if limit else self.playlist, 1):
    ...
    def get_albums(self) -> Dict[str, List[MusicMetadata]]:
        ...
        albums = {}
        for metadata in self.playlist:
            album_key = f"{metadata.album or '未知专辑'} - {metadata.artist or '未知艺术家'}"
