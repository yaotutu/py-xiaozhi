...
from .models import (
    Location, AddressComponent, GeocodeResult, POI, RouteResult, 
    RoutePath, RouteStep, WeatherForecast, WeatherInfo, DistanceResult,
    IPLocationResult, TransitResult, TransitRoute, TransitSegment,
    SearchResult, SearchSuggestion, BusLine
)
...
class AmapClient:
    ...
    async def regeocode(self, location: str) -> Dict[str, Any]:
        ...
        if data.get('status') != '1':
            raise Exception(f"RGeocoding failed: {data.get('info', data.get('infocode'))}")
    ...
    async def geocode(self, address: str, city: Optional[str] = None) -> List[GeocodeResult]:
        """地理编码"""
        ...
        if data.get('status') != '1':
            raise Exception(f"Geocoding failed: {data.get('info', data.get('infocode'))}")
    ...
    async def ip_location(self, ip: str) -> IPLocationResult:
        ...
        if data.get('status') != '1':
            raise Exception(f"IP Location failed: {data.get('info', data.get('infocode'))}")
    ...
    async def weather(self, city: str) -> WeatherForecast:
        ...
        if data.get('status') != '1':
            raise Exception(f"Get weather failed: {data.get('info', data.get('infocode'))}")
    ...
    async def search_detail(self, poi_id: str) -> POI:
        ...
        if data.get('status') != '1':
            raise Exception(f"Get poi detail failed: {data.get('info', data.get('infocode'))}")
    ...
    async def direction_walking(self, origin: str, destination: str) -> RouteResult:
        ...
        if data.get('status') != '1':
            raise Exception(f"Direction Walking failed: {data.get('info', data.get('infocode'))}")
    ...
    async def direction_driving(self, origin: str, destination: str) -> RouteResult:
        ...
        if data.get('status') != '1':
            raise Exception(f"Direction Driving failed: {data.get('info', data.get('infocode'))}")
    ...
    async def direction_bicycling(self, origin: str, destination: str) -> RouteResult:
        ...
        if data.get('errcode') != 0:
            raise Exception(f"Direction bicycling failed: {data.get('info', data.get('infocode'))}")
        
        return self._parse_route_result(data['data'])
    
    async def direction_transit(self, origin: str, destination: str, 
                               city: str, cityd: str) -> TransitResult:
        """公交路径规划"""
        params = {
            'origin': origin,
            'destination': destination,
        ...
        if data.get('status') != '1':
            raise Exception(f"Direction Transit failed: {data.get('info', data.get('infocode'))}")
        
        return self._parse_transit_result(data['route'])
    
    async def distance(self, origins: str, destination: str, 
                      distance_type: str = "1") -> List[DistanceResult]:
        """距离测量"""
        params = {
            'origins': origins,
            'destination': destination,
        ...
        if data.get('status') != '1':
            raise Exception(f"Distance failed: {data.get('info', data.get('infocode'))}")
    ...
    async def text_search(self, keywords: str, city: str = "", 
                         citylimit: str = "false") -> SearchResult:
        """关键词搜索"""
        params = {
            'keywords': keywords,
            'city': city,
        ...
        if data.get('status') != '1':
            raise Exception(f"Text Search failed: {data.get('info', data.get('infocode'))}")
        
        return self._parse_search_result(data)
    
    async def around_search(self, location: str, radius: str = "1000", 
                           keywords: str = "") -> SearchResult:
        """周边搜索"""
        params = {
            'location': location,
            'radius': radius,
        ...
        if data.get('status') != '1':
            raise Exception(f"Around Search failed: {data.get('info', data.get('infocode'))}")
    ...
    def _parse_search_result(self, data: Dict[str, Any]) -> SearchResult:
        ...
        suggestion = SearchSuggestion(
            ...
            cities=[{'name': city.get('name', '')} for city in suggestion_data.get('cities', [])]
        )
