...
def scan_installed_applications() -> List[Dict[str, str]]:
    ...
    system_apps = [
        ...
        {"name": "System Preferences", "display_name": "系统偏好设置", "path": "System Preferences", "type": "system"},
    ]
...
def _should_include_process(comm: str, command: str) -> bool:
    ...
    system_processes = {
        ...
        "loginwindow", "WindowServer", "SystemUIServer", "Dock", "Finder",
        "ControlCenter", "NotificationCenter", "WallpaperAgent", "Spotlight",
        "WiFiAgent", "CoreLocationAgent", "bluetoothd", "wirelessproxd",
        
        # 系统服务
        "com.apple.", "suhelperd", "softwareupdated", "cloudphotod",
        "identityservicesd", "imagent", "sharingd", "remindd", "contactsd",
        "accountsd", "CallHistorySyncHelper", "CallHistoryPluginHelper",
        
        # 驱动和扩展
        "AppleSpell", "coreaudiod", "audio", "webrtc", "chrome_crashpad_handler",
        "crashpad_handler", "fsnotifier", "mdworker", "mds", "spotlight",
        
        # 其他系统组件
        "automountd", "autofsd", "aslmanager", "syslogd", "ntpd", "mDNSResponder",
        "distnoted", "notifyd", "powerd", "thermalmonitord", "watchdogd",
    }
    ...
    if any(path in command_lower for path in [
        ...
        "/system/applications/utilities/", "/private/var/",
        "com.apple.", ".xpc/", ".framework/", ".appex/",
        "helper (gpu)", "helper (renderer)", "helper (plugin)",
        "crashpad_handler", "fsnotifier"
    ]):
        return False
    
    # 排除明显的系统服务
    if any(keyword in command_lower for keyword in [
        "xpcservice", "daemon", "agent", "service", "monitor",
        "updater", "sync", "backup", "cache", "log"
    ]):
        return False
    
    # 只包含用户应用程序
    user_app_indicators = [
        "/applications/", "/users/", "~/", ".app/contents/macos/"
    ]
