...
from .models import Recipe, Ingredient, Step, PaginatedResult
...
class RecipeClient:
            ...
            async with self.session.get(self.recipes_url) as response:
                ...
                for recipe_data in data:
                    try:
                        ...
                        logger.warning(f"解析菜谱失败: {recipe_data.get('name', 'Unknown')}, 错误: {e}")
                        continue
    ...
    def paginate_recipes(self, recipes: List[Recipe], page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        对菜谱列表进行分页.
    ...
    def paginate_simple_recipes(self, recipes: List[Recipe], page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        对菜谱列表进行分页，返回简化版数据.
    ...
    def paginate_name_only_recipes(self, recipes: List[Recipe], page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
    ...
    def search_recipes(self, recipes: List[Recipe], query: str, page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        搜索菜谱并分页返回结果.
    ...
    def get_recipes_by_category(self, recipes: List[Recipe], category: str, page: int = 1, page_size: int = 10) -> PaginatedResult:
        """
        根据分类获取菜谱并分页返回结果.
