...
def scan_installed_applications() -> List[Dict[str, str]]:
    ...
    system_apps = [
        ...
        {"name": "gnome-control-center", "display_name": "设置", "path": "gnome-control-center", "type": "system"},
    ]
...
def _should_include_app(display_name: str) -> bool:
    ...
    exclude_patterns = [
        ...
        # 系统工具
        "dconf", "gsettings", "xdg-", "desktop-file-",
        
        # 其他系统组件
        "help", "about", "preferences", "settings"
    ]
...
def _should_include_process(comm: str, command: str) -> bool:
    ...
    system_processes = {
        ...
        "systemd", "init", "kernel", "kworker", "kcompactd",
        
        # 系统服务
        "dbus", "networkd", "resolved", "logind", "udevd",
        "cron", "rsyslog", "ssh", "avahi", "cups",
        
        # 桌面环境服务
        "gnome-", "kde-", "xfce-", "unity-", "compiz",
        "pulseaudio", "pipewire", "wireplumber",
        
        # X11/Wayland
        "Xorg", "wayland", "weston", "mutter", "kwin",
    }
    ...
    if any(path in command_lower for path in [
        "/usr/libexec/", "/usr/lib/", "/lib/", "/sbin/",
        "/usr/sbin/", "/bin/systemd", "/usr/bin/dbus"
    ]):
        return False
    
    # 排除明显的系统服务
    if any(keyword in command_lower for keyword in [
        "daemon", "service", "helper", "agent", "monitor"
    ]):
        return False
    
    # 只包含用户应用程序
    user_app_indicators = [
        "/usr/bin/", "/usr/local/bin/", "/opt/", "/home/",
        "/snap/", "/flatpak/"
    ]
