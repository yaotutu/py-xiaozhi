...
async def get_cached_applications(force_refresh: bool = False) -> List[Dict[str, Any]]:
    ...
    if (not force_refresh and 
            _cached_applications is not None and 
            (current_time - _cache_timestamp) < _cache_duration):
        logger.debug(f"[AppUtils] 使用缓存的应用程序列表，缓存时间: {int(current_time - _cache_timestamp)}秒前")
    ...
    try:
        ...
        if result.get("success", False):
            ...
            logger.info(f"[AppUtils] 应用程序缓存已刷新，找到 {len(_cached_applications)} 个应用")
            return _cached_applications
        else:
            logger.warning(f"[AppUtils] 应用程序扫描失败: {result.get('message', '未知错误')}")
...
async def find_best_matching_app(app_name: str, app_type: str = "any") -> Optional[Dict[str, Any]]:
    """
    查找最佳匹配的应用程序.
    ...
    try:
        ...
        logger.info(f"[AppUtils] 找到最佳匹配: {best_app.get('display_name', best_app.get('name', ''))} (分数: {best_score})")
        return best_app
